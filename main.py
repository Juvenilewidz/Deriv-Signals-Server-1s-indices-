#!/usr/bin/env python3
"""
main.py — Dynamic Support & Resistance (CONFIRMED REJECTION ONLY)

Strict DSR strategy with confirmed rejection detection:
- MA1 (SMMA HLC3-9) and MA2 (SMMA Close-19) = Dynamic S/R  
- MA3 (SMA MA2-25) = Trend filter
- ONLY confirmed rejection candles at MA1/MA2 after candle close
- Rejection wick must be at least 1.5x body size AND 60% of total range
...
def is_confirmed_rejection_candle(candle):
    """
    Strict rejection detection - ONLY confirmed rejection patterns
    Requirements:
    ...
    # 1. Wick must be at least 1.5x body size
    # 2. Wick must be at least 60% of total range
...
# -------------------------
def get_trend_bias(ma1_val, ma2_val, ma3_val, current_price):
    """Determine trend bias based on MA arrangement and price position"""
    if not all(v is not None for v in [ma1_val, ma2_val, ma3_val]):
        return "UNDEFINED"

    # Price must be above MA3 for buy bias, below MA3 for sell bias
    if current_price > ma3_val and ma1_val > ma2_val:
        return "BUY_BIAS"
    elif current_price < ma3_val and ma1_val < ma2_val:
        return "SELL_BIAS"
    else:
        return "NO_BIAS"

# -------------------------
# MAIN Signal Detection (CONFIRMED ONLY)
# -------------------------
def detect_signal(candles, tf, shorthand):
    """
    Detect ONLY confirmed rejection signals - very strict criteria
    1. Wait for candle to close
    2. Check for confirmed rejection pattern (1.5x body, 60% range)
    3. Rejection must be AT MA1 or MA2 (actual contact, no tolerance)
    4. Must align with trend bias
    """
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # Step 1: Check if candle touched MA1 or MA2 level
    ...
    # Step 2: Check for CONFIRMED rejection pattern
    is_rejection, rejection_type = is_confirmed_rejection_candle(current_candle)
    if not is_rejection:
        if DEBUG:
            print(f"No confirmed rejection for {shorthand}")
        return None

    # Step 3: Get trend bias
    trend_bias = get_trend_bias(current_ma1, current_ma2, current_ma3, current_candle["close"])
    if trend_bias == "NO_BIAS" or trend_bias == "UNDEFINED":
        if DEBUG:
            print(f"No clear trend bias for {shorthand}")
        return None

    # Step 4: Match rejection type with bias
    signal_side = None
    
    if trend_bias == "BUY_BIAS" and rejection_type == "LOWER_REJECTION":
        # Bullish market, lower rejection at MA = buy signal
        signal_side = "BUY"
    elif trend_bias == "SELL_BIAS" and rejection_type == "UPPER_REJECTION":
        # Bearish market, upper rejection at MA = sell signal  
        signal_side = "SELL"
    else:
        if DEBUG:
            print(f"Rejection type {rejection_type} doesn't match bias {trend_bias}")
        return None

    if DEBUG:
        print(f"CONFIRMED DSR SIGNAL: {shorthand} {signal_side} - {rejection_type} at {ma_level}")
...
# -------------------------
# Chart Generation (with ALL MAs plotted)
# -------------------------
def create_signal_chart(signal_data):
    ...
    # Title
    bias_emoji = "📈" if "BUY" in signal_data["bias"] else "📉"
    ax.set_title(f"{signal_data['symbol']} - CONFIRMED {signal_data['side']} DSR Signal {bias_emoji}", 
...
# -------------------------
def run_analysis():
    """Analysis with CONFIRMED rejection detection only"""
    signals_found = 0
    ...
    for shorthand, deriv_symbol in SYMBOL_MAP.items():
        try:
            ...
            bias_emoji = "📈" if "BUY" in signal["bias"] else "📉"
            caption = (f"🎯 CONFIRMED {signal['symbol']} {tf}s - {signal['side']} SIGNAL\n"
                      f"{bias_emoji} Bias: {signal['bias'].replace('_', ' ')}\n" 
                      f"🎨 Pattern: CONFIRMED {signal['pattern']}\n"
                      f"📍 Level: {signal['ma_level']} Contact\n"
