...
def detect_signal(candles, tf, shorthand):
    """Enhanced DSR with confirmed rejection signals"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # Skip if this might be a forming candle (in real-time)
    # In backtesting, all candles are closed, but in live trading we need to ensure closure
    ...
    # Test for MA rejection
    has_rejection, ma_level, pattern_type = test_ma_rejection(current_candle, current_ma1, current_ma2)
    
    if not has_rejection:
        return None
    
    # Match signal direction with trend bias and rejection pattern
    signal_side = None
    
    if trend_bias == "BUY_BIAS":
        # In uptrend, look for lower rejection (buying opportunity)
        if pattern_type == "LOWER_REJECTION":
            signal_side = "BUY"
    
    elif trend_bias == "SELL_BIAS":
        # In downtrend, look for upper rejection (selling opportunity) 
...
def run_analysis():
    ...
    for shorthand, deriv_symbol in SYMBOL_MAP.items():
        try:
            ...
            caption = (f"🎯 {signal['symbol']} {tf}s - {signal['side']} CONFIRMED REJECTION\n"
                      f"{bias_emoji} Trend Bias: {signal['bias'].replace('_', ' ')}\n" 
                      f"🔄 Pattern: {signal['pattern']} at {signal['ma_level']}\n"
